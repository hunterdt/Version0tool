...
$.fn.progressTracker = function (options) {
    ...
    var settings = $.extend({
        // HORIZONTAL tracker -->
        horTracker: false, // displays a HORIZONTAL scroll progress tracker
        ...
        // VERTICAL tracker -->
        verTracker: true, // displays a VERTICAL scroll progress tracker
        verPosition: 'right', // creates the VERTICAL tracker on the left side of the page if set to 'left', at the right side if set to 'right'
        verStyle: 'beam', // Sets the style of the VERTICAL tracker
        // ---> 'beam', 'fill'
        verColor: 'blue', // Sets the color of the default gradient for the VERTICAL tracker
        // ---> 'red', 'blue', 'green', 'orange', 'silver'
        ...
        // General -->
        mobileThreshold: 957, // sets the viewport width below which a device is considered 'small screen' for the rest of the options
        trackAllHeadlines: true, // if set to true, all headlines (h1, h2, h3 etc.) within .spt-trackThis will be converted to tracker titles (if horTitles/verTitles is also true), if set to false only h3-headlines will be tracked
        ...
        trackViewportOnly: true, // turns section stops and titles inactive again, when the corresponding section leaves the viewport
        // <-- General
    }, options);
    // <-- Default options
    ...
    $(document).ready(function () {
        ...
        function setScrollStops() {
            trackedArea.find('.spt-trackThis').each(function (index) {
                ...
                $(this).children('.spt-sectionTitle:first').attr({
                    id: 'SectionHeadline' + sectionId
                });
